...
#define M2M_WIFI_SCAN_DEFAULT_SLOT_TIME \
	(30) // Default duration in miliseconds of a scan slots performed by the WINC board
...
typedef struct {
	...
	uint8_t
		au8Passwd[M2M_WIFI_PASSWORD_1X_MAX]; // Password corresponding to the user name. It must be Null terminated string
...
typedef struct {
	tuniM2MWifiAuth
		uniAuth;       // Union holding all possible authentication parameters corresponding the current security types
