...
int8_t hif_send(uint8_t groupId, uint8_t opCode, uint8_t *p_ctrlBuf, uint16_t ctrlBufSize, uint8_t *p_dataBuf,
	uint16_t dataSize, uint16_t dataOffset)
{
	int8_t ret;
	t_hifHdr hif;
	...
	if (reg & 0x1) /* New interrupt has been received */
	{
		...
		if (size > 0) {
			...
			if (M2M_REQ_GROUP_WIFI == hifHdr.groupId) {
				WifiInternalEventHandler(
					hifHdr.opCode, hifHdr.length - M2M_HIF_HDR_OFFSET, address + M2M_HIF_HDR_OFFSET);
			} else if (REQ_GROUP_IP == hifHdr.groupId) {
				SocketInternalEventHandler(
					hifHdr.opCode, hifHdr.length - M2M_HIF_HDR_OFFSET, address + M2M_HIF_HDR_OFFSET);
			} else if (REQ_GROUP_OTA == hifHdr.groupId) {
				OtaInternalEventHandler(
					hifHdr.opCode, hifHdr.length - M2M_HIF_HDR_OFFSET, address + M2M_HIF_HDR_OFFSET);
...
void hif_receive(uint32_t address, uint8_t *p_buf, uint16_t size, uint8_t isDone)
{
	...
	if (size > g_hifContext.rxSize) {
		dprintf(
			"APP Requested Size is larger than the receive buffer size <%d><%d>\r\n", size, (int)g_hifContext.rxSize);
	...
	if (reg & 0x1) /* New interrupt has been received */
	{
		...
		if (size > 0) {
			...
			if (REQ_GROUP_IP == hifHdr.groupId) {
				SocketInternalEventHandler_Pic18WaiteHttpSend(
					hifHdr.opCode, hifHdr.length - M2M_HIF_HDR_OFFSET, address + M2M_HIF_HDR_OFFSET);
			}
