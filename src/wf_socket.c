...
void ReadSocketData(
	SOCKET sock, t_socketRecv *p_socketRecv, uint8_t socketMsgId, uint32_t startAddress, uint16_t readCount)
{
	if ((readCount > 0) && (g_sockets[sock].p_userBuf != NULL) && (g_sockets[sock].userBufSize > 0) &&
		(g_sockets[sock].isUsed == 1)) {
		...
		for (; count < socketCount; count++) {
			...
			if (p_sock->isUsed == 0) {
				...
				if (u8Flags & SOCKET_FLAGS_SSL) {
					...
					SOCKET_REQUEST(
						SOCKET_CMD_SSL_CREATE, (uint8_t *)&sslCreateConfig, sizeof(t_sslSocketCreateCmd), 0, 0, 0);
				}
				break;
			}
		}
...
int8_t sendto(SOCKET sock, void *buf, uint16_t len, uint16_t flags, struct sockaddr *to, uint8_t tolen)
{
	...
	if ((sock >= 0) && (buf != NULL) && (len <= SOCKET_BUFFER_MAX_LENGTH) && (g_sockets[sock].isUsed == 1)) {
		if (g_sockets[sock].isUsed) {
			...
			retVal = SOCKET_REQUEST(SOCKET_CMD_SENDTO | REQ_DATA_PKT, (uint8_t *)&sendToConfig, sizeof(t_sendCmd), buf,
				len, UDP_TX_PACKET_OFFSET);
...
Date
        9 September 2014
*********************************************************************/
int8_t setsockopt(SOCKET sock, uint8_t level, uint8_t optname, const void *optval, uint16_t optlen)
{
	int8_t retVal = SOCK_ERR_INVALID_ARG;
		...
		if (g_sockets[sock].sslFlags & SSL_FLAGS_ACTIVE) {
			if (option == SO_SSL_BYPASS_X509_VERIF) {
				...
				if (optionLength < M2M_HOSTNAME_MAX_SIZE) {
					...
					if (SOCKET_REQUEST(SOCKET_CMD_SSL_SET_SOCK_OPT, (uint8_t *)&cmd, sizeof(t_sslSetSockOptCmd), 0, 0,
							0) == M2M_ERR_MEM_ALLOC) {
						retVal = SOCKET_REQUEST(SOCKET_CMD_SSL_SET_SOCK_OPT | REQ_DATA_PKT, (uint8_t *)&cmd,
							sizeof(t_sslSetSockOptCmd), 0, 0, 0);
					}
					retVal = SOCK_ERR_NO_ERROR;
				} else {
...
#if defined(__XC8)
extern void m2m_socket_handle_events_Pic18WaiteHttpSend(
	SOCKET sock, t_m2mSocketEventType eventCode, t_socketEventData *p_eventData);
