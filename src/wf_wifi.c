...
static void ConnectInternal(
	const char *pcSsid, uint8_t u8SsidLen, uint8_t u8SecType, void *pvAuthInfo, uint16_t channel, uint8_t noSaveCred);
...
static bool isConnectionParamsValid(
	const char *pcSsid, uint8_t u8SsidLen, uint8_t u8SecType, void *pvAuthInfo, uint16_t channel);
...
static bool BootRomStateMachine(void)
{
	...
	case BOOT_ROM_CHECK_REV:
		driverVersion = MAKE_VERSION_INFO(FIRMWARE_RELEASE_VERSION_MAJOR_NO, FIRMWARE_RELEASE_VERSION_MINOR_NO,
			FIRMWARE_RELEASE_VERSION_PATCH_NO, M2M_FIRMWARE_VERSION_MAJOR_NO, M2M_FIRMWARE_VERSION_MINOR_NO,
	...
	if (p_revision != NULL) {
		...
		if (reg != 0) {
			...
			if (reg != 0) {
				...
				curr_firm_ver =
					MAKE_VERSION(p_revision->u8FirmwareMajor, p_revision->u8FirmwareMinor, p_revision->u8FirmwarePatch);
				curr_drv_ver = MAKE_VERSION(
					M2M_FIRMWARE_VERSION_MAJOR_NO, M2M_FIRMWARE_VERSION_MINOR_NO, M2M_FIRMWARE_VERSION_PATCH_NO);
				min_req_drv_ver =
					MAKE_VERSION(p_revision->u8DriverMajor, p_revision->u8DriverMinor, p_revision->u8DriverPatch);
...
#endif
	hif_send(M2M_REQ_GROUP_WIFI, CFG_REQ_CUST_INFO_ELEMENT | REQ_DATA_PKT, (uint8_t *)pau8M2mCustInfoElement,
...
#endif
	hif_send(M2M_REQ_GROUP_WIFI, CFG_REQ_SET_SCAN_OPTION, (uint8_t *)ptstrM2MScanOption, sizeof(tstrM2MScanOption),
		NULL, 0, 0);
}
...
{
#if defined(M2M_ENABLE_ERROR_CHECKING)
	if ((scanRegion != M2M_WIFI_NORTH_AMERICA_REGION) && (scanRegion != M2M_WIFI_EUROPE_REGION) &&
		(scanRegion != M2M_WIFI_NORTH_ASIA_REGION)) {
	...
	hif_send(
		M2M_REQ_GROUP_WIFI, CFG_REQ_SET_SCAN_REGION, (uint8_t *)&strScanRegion, sizeof(tstrM2MScanRegion), NULL, 0, 0);
}
...
void m2m_wifi_req_hidden_ssid_scan(uint8_t ch, uint8_t *p_ssidList)
{
	...
	if (apNum <= M2M_WIFI_MAX_HIDDEN_SITES) {
		...
		tmp.u8ChNum = ch;
		hif_send(M2M_REQ_GROUP_WIFI, CFG_REQ_SCAN_SSID_LIST | REQ_DATA_PKT, (uint8_t *)&tmp, sizeof(tstrM2MScan),
			p_ssidList, listSize, sizeof(tstrM2MScan));
		g_scanInProgress = 1;
	}
}
...
void m2m_wifi_set_device_name(char *pu8DeviceName, uint8_t u8DeviceNameLength)
{
	...
	hif_send(M2M_REQ_GROUP_WIFI, M2M_WIFI_REQ_SET_DEVICE_NAME, (uint8_t *)&deviceName, sizeof(tstrM2MDeviceNameConfig),
		NULL, 0, 0);
}
...
void m2m_wifi_start_provision_mode(
	tstrM2MAPConfig *pstrM2MAPConfig, char *p_httpServerDomainName, uint8_t bEnableHttpRedirect)
{
	...
	//  Stop Scan if it is ongoing.
	g_scanInProgress = false;
	hif_send(M2M_REQ_GROUP_WIFI, CFG_REQ_START_PROVISION_MODE | REQ_DATA_PKT, (uint8_t *)&strProvConfig,
		sizeof(tstrM2MProvisionModeConfig), NULL, 0, 0);
}
...
void m2m_wifi_set_power_profile(uint8_t u8PwrMode)
{
	...
	hif_send(
		M2M_REQ_GROUP_WIFI, CFG_REQ_SET_POWER_PROFILE, (uint8_t *)&wincPowerMode, sizeof(tstrM2mPwrMode), NULL, 0, 0);
}
...
void m2m_wifi_set_tx_power(uint8_t u8TxPwrLevel)
{
	...
	hif_send(
		M2M_REQ_GROUP_WIFI, CFG_REQ_SET_TX_POWER, (uint8_t *)&wincPowerLevel, sizeof(tstrM2mTxPwrLevel), NULL, 0, 0);
}
...
void m2m_wifi_set_battery_voltage(uint16_t batVoltx100)
{
	...
	hif_send(M2M_REQ_GROUP_WIFI, CFG_REQ_SET_BATTERY_VOLTAGE, (uint8_t *)&batVoltage, sizeof(tstrM2mBatteryVoltage),
		NULL, 0, 0);
}
...
static bool isValidApParameters(const tstrM2MAPConfig *apConfig)
{
	...
	if (apConfig->u8SecType != M2M_WIFI_SEC_OPEN) {
		if (apConfig->u8SecType == M2M_WIFI_SEC_WEP) {
			...
			if ((strlen((char *)apConfig->au8WepKey) <= 0) ||
				(strlen((char *)apConfig->au8WepKey) > WEP_104_KEY_STRING_SIZE)) {
...
static int8_t ValidateScanOptions(tstrM2MScanOption *ptstrM2MScanOption)
{
	...
	if ((ptstrM2MScanOption->u8ProbesPerSlot == 0) ||
		(ptstrM2MScanOption->u8ProbesPerSlot > M2M_WIFI_SCAN_DEFAULT_NUM_PROBE)) {
...
static bool isValidScanChannel(int channel)
{
	return (((channel >= M2M_WIFI_CH_1) && (channel <= M2M_WIFI_CH_14)) || (channel == M2M_WIFI_CH_ALL));
}
	...
	const char *pcSsid, uint8_t u8SsidLen, uint8_t u8SecType, void *pvAuthInfo, uint16_t channel)
{
	...
	if (u8SecType == M2M_WIFI_SEC_WEP) {
		...
		if (keyIndex >= M2M_WIFI_WEP_KEY_MAX_INDEX) {
			...
			return false;
		} else if ((p_wep->u8KeySz != M2M_WIFI_WEP_40_KEY_STRING_SIZE + 1) &&
			(p_wep->u8KeySz != WEP_104_KEY_STRING_SIZE + 1)) {
